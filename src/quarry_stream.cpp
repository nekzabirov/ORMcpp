...
QuarryStream
QuarryStream::select(const std::initializer_list<std::string_view>& keys)
{
    std::stringstream ss;
...
QuarryStream
QuarryStream::select(const std::vector<std::string>& keys)
{
    std::stringstream ss;
...
QuarryStream&&
QuarryStream::leftJoin(const std::string_view& table,
...
QuarryStream&&
QuarryStream::rightJoin(const std::string_view& table,
                        const std::string_view& as,
                        const Conditional&&     on) &&
{
    append(" RIGHT JOIN ");
...
QuarryStream&&
QuarryStream::innerJoin(const std::string_view& table,
                        const std::string_view& as,
                        const Conditional&&     on) &&
{
    append(" INNER JOIN ");
